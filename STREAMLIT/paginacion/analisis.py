...
def page_analisis():
    ...
    st.markdown(
        ...
        "-Escoge bien la botonera situada en la parte izquierda de la página para filtrar bien.\n\n"
    ...
    st.write(f"Total ofertas: {len(df_f3) if mod_teletrabajo else len(df_f)}\n")
        # CheckBox
    if st.checkbox(label = "Pulse para ver los enlaces"):
    ...
    for i in geojson_data['features']:
        ...
        geom = shape(i['geometry'])  # Usamos shapely para calcular centroide
...
# ── 3) Habilidades solicitadas por año
    ...
    db_conectada.close()
    # Limpiamos la columna fecha para poder filtrarla por año
...
################################################
    ...
    # Paso 3: marcar como outlier si z_score > 3 o < -3
...
################################################
    ...
    # De esta manera el gráfico quedará mucho claro
