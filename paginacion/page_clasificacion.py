...
def page_clasificacion():
    ...
    fila_escalada_df = pd.DataFrame(fila_escalada, columns=nueva_fila.columns)
    pred = modelo.predict(fila_escalada_df)[0]
    proba = modelo.predict_proba(fila_escalada_df)
